/* Race medal rank — adapted from OpenCity race/medals.js */
export const MEDAL_KEY = 'roaddrive.medals';
...
export function saveMedals(m) {
  try { localStorage.setItem(MEDAL_KEY, JSON.stringify(m)); } catch { /* private */ }
}

export function pointsForPlace(pos) {
  // Podium prizes only: 1st gold, 2nd silver — no award for 3rd/4th.
  if (pos === 1) return 3;
  if (pos === 2) return 2;
  return 0;
}

export function prizeForPlace(pos) {
  if (pos === 1) return { prize: 'GOLD', color: '#f0b429' };
  if (pos === 2) return { prize: 'SILVER', color: '#c5c8ce' };
  return { prize: null, color: '#8a857a' };
}
...
export function awardPlace(medals, pos) {
  ...
  const { prize, color: prizeColor } = prizeForPlace(pos);
  ...
  return {
    ...
    rank: medals.rank,
    name: rank.name,
    color: prize ? prizeColor : rank.color,
